...
from database import (
    ...
    add_service, get_services, get_service_categories,
...
# Query parameters (voor Stripe en publieke pagina)
...
# -----------------------------
    ...
    if not is_company_paid(company_id):
        ...
        try:
            checkout_url = create_checkout_session(company_id, get_company_by_id(company_id)[2], company_name)
    ...
    # Dashboard Tabs
    st.markdown(
        f"<h2 style='color:#FFD166;text-align:center;'>Welkom, {company_name}</h2>",
    ...
    with tab_dash:
        st.markdown(
            ...
            Voeg hieronder je diensten, tijden en beschikbaarheid toe. 
            Alles wat je hier instelt wordt automatisch zichtbaar op je publieke boekingspagina.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if services.empty:
            ...
            for cat in services["category"].dropna().unique():
                ...
                for _, row in sub.iterrows():
                    if st.checkbox(f"{row['name']} — €{row['price']:.2f} • {int(row['duration'])} min",
                                   key=f"pv_{row['id']}"):
...
# -----------------------------
    ...
    st.markdown(f"<h2 style='text-align:center;color:#FFD166;'>Boek bij {company_name}</h2>", unsafe_allow_html=True)
